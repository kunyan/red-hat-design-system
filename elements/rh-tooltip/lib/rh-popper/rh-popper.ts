...
import styles from './rh-popper.css';
import { getRandomId } from '@patternfly/pfe-core/functions/random.js';
import { Position } from './rh-popper-types';

/**
 * Wrapper for popper.js for rh-tooltip.js. 
 * 
 * Can be moved out of this library if we want for multiple packages. 
 *
 * @summary Organizes content in a contained view on the same page
 *
 */

@customElement('rh-popper') @pfelement()
export class RhPopper extends LitElement {
  static readonly version = '{{version}}';

  static readonly styles = [styles];

  @property({ type: String, reflect: true }) position: Position = Position.TOP;

  @property({ type: Boolean, reflect: true, attribute: 'is-open' }) isOpen = true;

  @property({ type: Array, reflect: true }) offset = [0, 18];

  private _id = `${RhPopper.name}-${getRandomId()}`;
